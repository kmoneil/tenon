...
// UnicodeVersion. Upgrading github.com/rivo/uniseg, or regenerating the
// normalization tables from another version, can move one of them, which
// changes which strings are equal or how long they are. That must be a
// deliberate change of UnicodeVersion, never a silent one.
//
// Neither source follows the toolchain a consumer builds with. The
// normalization data is this package's own, generated by tools/unigen, and
// segmentation data comes from a pinned module version.
func TestConformance_ST003_UnicodeVersion(t *testing.T) {
	conformance.Covers(t, "ST-003")
	if tablesVersion != UnicodeVersion {
		t.Errorf("the normalization data is Unicode %s, but UnicodeVersion is %s", tablesVersion, UnicodeVersion)
	}
